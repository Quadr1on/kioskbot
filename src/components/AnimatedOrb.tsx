...
function ParticleSphere({ state, analyser }: { state: string, analyser?: AnalyserNode | null }) {
  ...
  const color = useMemo(() => {
    switch (state) {
      case 'recording': return '#E53E3E'; // SIMS accent red
      case 'processing': return '#ED8936'; // SIMS accent orange
      case 'greeting':
      case 'speaking': return '#3399FF'; // SIMS blue light
      case 'listening': return '#00A89D'; // SIMS accent teal
      case 'idle':
      default: return '#0066CC'; // SIMS blue
    }
  }, [state]);

  useFrame((stateContext: any) => {
